...
function CampaignPopup({ campaign, timeLeft, gradient, onClose, visible }: {
  ...
  return (
    ...
    >
      <div className={`relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transition-all duration-300 ${
        ...
        <div className="px-6 pt-5 pb-2">
          <p className="text-center text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-4">
          ...
          <div className="flex items-start justify-center gap-2">
            {timeLeft.days > 0 && (
              <>
                ...
                <span className="text-2xl font-bold text-gray-300 dark:text-gray-600 mt-2">:</span>
              </>
            )}
        ...
        <div className="px-6 pb-6 pt-4">
          ...
          <button
            onClick={onClose}
            className="block w-full text-center mt-2 py-2 text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
          >
            Şimdi Değil
          </button>
        </div>
...
function CountdownCard({ value, label, gradient }: { value: number; label: string; gradient: string }) {
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="text-[9px] text-gray-400 mt-1.5 uppercase tracking-widest">{label}</span>
    </div>
  );
}
